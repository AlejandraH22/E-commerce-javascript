...
const pantalonGamuza = new Prendas("pantalón de gamuza", "3/4", "38 y 1/2", "pantalones", 1200, 50);
const pantalonLargo = new Prendas("pantalón largo", "pantalon", "40", "pantalones", 8000, 81);
...
const descuento = () => {
    let codigo = prompt("decinos tu codigo de descuento")
    ...
    if (codigo === "remera") {
        ...
        alert("el codigo de descuento es invalido")
    }
};
...
while (consulta === "si"){
    ...
    let cantidadPrenda = (prompt("Cuantas prendas desea comprar")).toLowerCase()
        for (let el of stock){
        if (el.nombre === prenda){
    botonCompra(el, cantidadPrenda);

     carrito.map((el) => {alert(`agregaste ${el.producto} a tu carrito`)})
    }
}
consulta = prompt("desea realizar otra compra")
};

let consulta2 = (prompt("Querés conocer saber cuantos productos estas llevando carrito")).toLowerCase();

if (consulta2 === "si"){
    
   let cantidad = carrito.reduce((acc, el) => acc + el.cantidades, 0)
   alert (`Llevás en tu carrito ${cantidad} productos con un total de ${total()}`)
